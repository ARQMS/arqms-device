...
void GuiUpdaterTask::onHandleEvent(EventId eventId, Deserializer* pEvent) {
    switch (eventId) {
        ...
        case EventIdentifiers::SENSOR_STATUS_EVENT: 
...
void GuiUpdaterTask::onHandleTimer(const TimerId timerId) {
    ...
    else if (m_pDelayTimer->id == timerId) {
        onHandleDelay();
    }
}
...
void GuiUpdaterTask::onHandleDelay() {
    disable();
}
...
void GuiUpdaterTask::disable() {
    ...
    DisplayInfoEvent event(false);
    Control.send(EventIdentifiers::DISPLAY_INFO_EVENT, &event);
}
